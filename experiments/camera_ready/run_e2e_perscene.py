"""Per-scene variant of the end-to-end sweep.

Identical to experiments/e2e_harness/run_e2e_sweep.py, but additionally
writes one row per (detector, classifier, scene) with ground-truth, matched,
and correct counts, so that scene-level bootstrap analysis can quantify the
sampling uncertainty of the 20-scene stress set.
...
Usage:
    python run_e2e_sweep.py --lam 0.10 --surprise-to neutral \
        --out results/camera_ready/perscene.csv
"""
...
def main() -> None:
    ...
    for cname in args.classifiers:
        ...
        for dname in args.detectors:
            ...
            correct = matched = 0
            cls_lats, e2e_lats = [], []
            for si, (s, pairs) in enumerate(zip(scenes, dc["per_scene"])):
                scene_cls_ms = 0.0
                s_correct = s_matched = 0
                ...
                matched += s_matched
                scene_rows.append({
                    "detector": dname, "classifier": cname, "scene": si,
                    "n_gt": len(s["gt_groups"]), "n_matched": s_matched,
                    "n_correct": s_correct,
                })
    ...
    if args.out:
        ...
        pd.DataFrame(scene_rows).to_csv(
            out.with_name(out.stem + "_per_scene.csv"), index=False)
        print(f"wrote {out} (+ per-scene table)")
